...
const MintProfile = () => {
  ...
  const sendTxToBlockchain = async (metadata) => {
    try {
      setTxStatus("Sending mint transaction to Fantom Blockchain.");
      ...
      setErrorMessage("Failed to send tx to Fantom Testnet.");
      console.log(error);
    }
  };
  ...
  const previewNFT = (metaData, mintNFTTx) => {
    ...
    setMetaDataURl(getIPFSGatewayURL(metaData.url));
    setTxURL(`https://testnet.ftmscan.com/tx/${mintNFTTx.hash}`);
